"""
HAGRPO: Heterogeneous-Agent Group Relative Policy Optimization

This implements HAGRPO, which combines:
...
@dataclass
class HAGRPOConfig(TrainingArguments):
    """
    Configuration for HAGRPO training, inheriting from TrainingArguments.
    ...
    per_device_train_batch_size: int = field(
        default=1,
        metadata={"help": "Per-device batch size (must be 1 for HAGRPO)."},
    )
...
class NodeSample:
    ...
    node_env_step: int
    # HAGRPO specific: store old log probs for importance ratio calculation
    old_log_probs: Optional[List[float]] = None


class HAGRPOTrainer:
    """
    Heterogeneous-Agent Group Relative Policy Optimization Trainer (HAGRPO).
    
    Key features:
    - Sequential agent updates following HAPPO's scheme
    - Importance ratio accumulation (M factor) across agents
    - GRPO-style group-relative advantage estimation
    ...
    def __init__(
        ...
        if not torch.cuda.is_available():
            raise RuntimeError(
                "GPU not found. HAGRPOTrainer requires GPU for training."
            )
        ...
        # Training arguments
        self.args = args if args is not None else HAGRPOConfig()
        self.env_step = 0
        ...
        if self.args.per_device_train_batch_size != 1:
            raise ValueError("HAGRPO requires per_device_train_batch_size to be 1.")
    ...
    def _init_wandb(self):
        ...
        if not self.wandb_initialized:
            ...
            if self.args.num_turns == 1:
                wandb_name = self.wandb_config.get("name", "test-hagrpo")
            else:
                wandb_name = self.wandb_config.get("name", "test-mt-hagrpo")
    ...
    def train(self, **kwargs):
        """
        Main training loop implementing HAGRPO with sequential agent updates.
        """
        ...
        for epoch in range(0, int(self.args.num_train_epochs)):
            ...
            for batch_idx, batch in it:
                ...
                batch_item = batch[0]
                # HAGRPO training step with sequential updates
                batch_loss, _batch_stats = self._train_step_hagrpo(
                    batch_item,
                    epoch_turn_rewards,
                    epoch_turn_returns,
                    **kwargs,
    ...
    def _train_step_hagrpo(
        ...
        **kwargs,
        ...
        """
        HAGRPO training step with sequential agent updates.
        
        Key differences from MAGRPO:
        1. Agents are updated sequentially
        2. M factor accumulates importance ratios across agents
        3. PPO-clip objective is used with M factor
        """
        ...
        # Storage for completions and log probs per agent
        all_completions_data: List[Dict[str, Any]] = []
        all_old_log_probs: List[List[torch.Tensor]] = []
        ...
        # HAGRPO: Sequential update with importance ratio accumulation
        self._sequential_update_hagrpo(all_nodes)
    ...
    def _sequential_update_hagrpo(self, all_nodes: List[Dict[str, Any]]):
        """
        Perform sequential agent updates following HAPPO scheme.
        ...
        for node in all_nodes:
            ...
            returns_tensor = torch.tensor(returns_vec, dtype=torch.float, device=device)
    ...
    def _process_buffer(self, agent_idx: int, buffer: List[NodeSample]) -> None:
        """Process buffered samples - for HAGRPO this is simplified since updates happen in _sequential_update_hagrpo."""
        if not buffer:
            return
